...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email: email }).select('+password'); //{email} the same
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forot your password? Submit a PATCH request with you new password and passwordConfirm to: ${resetURL}.\nIf you didn't forget your password, please ignore this email`;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({
    ...
    passwordResetExpires: { $gt: Date.now() }, //compares the expirates time with our real time if it is greater it should find user
  });
